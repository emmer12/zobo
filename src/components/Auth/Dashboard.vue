<template>
  <div style="z-index:1;position:relative" v-if="user">
    ...
    <div class="email-confirm" v-if="user">
      ...
      <small>Didn't get anything? <span @click="resendEmail" class="primary--text font-weight-bold " style="cursor:pointer" >Resend email</span> of check your spam folder thanks.</small>
    </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    resendEmail(){
         this.$store.dispatch("resendVerifyEmail").then(() => {
          alert('sent')
         })
         }
  },
